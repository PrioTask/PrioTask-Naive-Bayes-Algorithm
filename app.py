...
client = MongoClient('mongodb+srv://naivepriotask:admin2023@priotask.fi0sig5.mongodb.net/?retryWrites=true&w=majority')
...
def logout():
    ...
    logout()
    return jsonify({"message": f"{result.modified_count} user(s) logged out successfully", "logged_out_users": online_user_list}, redirect("http://localhost:5173/login")) 
...
def data():
    if request.method == 'POST':
        ...
        def send_email(subject, recipient_email, message_template, name_placeholder='Recipient Name'):
            ...
            # Replace {name} with the recipient's name (you can change this to the actual name)
            ...
            try:
                ...
                print(f"Error: {e}")
        def send_delayed_email(subject, recipient_email, message_template, name_placeholder='Recipient Name'):
                
                
                # Delay the email by 10 seconds
                time.sleep(2700)
                subject = f"PrioTask due: {taskName}"
                # Prepare the message with the provided template
        ...
        def send_email_at_deadline(subject, recipient_email, message_template, deadline_date, name_placeholder='Recipient Name'):
            ...
            if days_until_deadline == 1:  # Send the email one day before the deadline
                msg_thread = threading.Thread(target=send_email, args=(subject, recipient_email, message_template, name_placeholder))
                msg_thread.start()

            elif days_until_deadline == 0:  # Send the email after 10 seconds
                delayed_thread = threading.Thread(target=send_delayed_email, args=(subject, recipient_email, message_template, name_placeholder))
        ...
        # Define multiple message templates below (copy and paste your templates here)
        message_templates = [
            f"""
            <html>
            <body>
        ...
        # Randomly select a message template to send
        ...
        # send_email(subject, userEmail, random_message_template)
        # Calculate the selected deadline date as before
        selected_deadline = datetime.strptime(deadline, '%Y-%m-%d').date()

        # Schedule the email to be sent at the desired time
